...
def queryData(data,PN=None):
    ...
    left join sap_reporting.plants as d on d.plant = a.werks 
    where PN1 in {PN}
    """# query top level PN production info
    ...
    client_store_temporary_credential=True, # Only if installing secure-local-storage to avoid reopening tabs
        ...
        # df_individualpn = cursor1.fetch_pandas_all()
    return df_toplevelpn #,df_externalcost,df_individualpn
...
if __name__== '__main__':
    ...
    data_excel.to_excel(f'data/{PN}_final_table.xlsx',index =False)
